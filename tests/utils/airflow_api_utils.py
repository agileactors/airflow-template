...
import allure
from airflow_client.client.api import dag_api, dag_run_api, import_error_api
...
class AirflowApiUtils:
    ...
    def run_dag(self, dag_id: str):
        with allure.step(f"Triggering DAG with id {dag_id}"):
    ...
    def wait_dag(self, dag_id: str, dag_run_id: str):
        with allure.step(f"Waiting DAG with id {dag_id}"):
    ...
    def list_dag_runs(self, dag_id: str) -> DAGRunCollection:
        with allure.step(f"List DAG runs of DAG with id {dag_id}"):
            try:
    ...
    @allure.step("Delete all DAG runs")
